...
![Some example output](images\Wine_Install.png)
...
### Linux
As mentioned earlier, Wine will run on most if not any common Linux distribution and version. But due to reasons we likely cannot control such as kernel versions the Meterpriter shell code that will be deployed will work better on some Linux distributions and versions.
...
* Windows specific attacks and resources may not be accessible (They, and their specific conditions may not exist on the Linux Distribution) ex. SAM database.
...
**Ubuntu**
* This is the Linux Distribution vChat and later Meterpriter through Wine appears to work the best on.
